...
class BLSTMPunctuator(object):
    ...
    def __init__(self, input_batch, label_batch, seq_len, is_training, config, use_attention=True):
        ...
        # if use_attention:
            # rnn_cell = tf.nn.rnn_cell.BasicRNNCell(hidden_size*2)
            # with tf.variable_scope("RNN"):
            #     output, state = tf.nn.dynamic_rnn(rnn_cell, outputs,
            #                            sequence_length=seq_len,
            #                            dtype=tf.float32)
            # maxlen = tf.cast(tf.reduce_max(seq_len), tf.int32)
            # rnn_output = tf.reshape(outputs, [batch_size, -1])
            # outputs = tf.layers.dense(inputs=rnn_output, units=maxlen*hidden_size*2,
